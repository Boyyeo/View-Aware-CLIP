...
if __name__ == "__main__":
    ...
    parser.add_argument("--subject_name", type=str, default=None,required=True)
    ...
    set_seed(args.seed)
    images = []
    view_prompt = ['front','left','back','right']
    os.makedirs('mvdream_result_eval/{}'.format(args.subject_name),exist_ok=True)
    for j in range(3):
        ...
        for k, view in enumerate(view_prompt):
            current_img = img[k]
            Image.fromarray(current_img).save("mvdream_result_eval/{}/{}_sample_{}_view_{}.png".format(args.subject_name,args.text,j,view))
