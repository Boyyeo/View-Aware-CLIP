...
from prompt_clip import PromptCLIPModel
...
class ModelArguments:
    ...
    prompt_tuning: bool = field(
        default=False, metadata={"help": "Whether to use prompt tuning approach to finetune."}
    )
...
class DataTrainingArguments:
    ...
    view_data_csv_path: str = field(default=None, metadata={"help": "path for view dataset csv"})
    view_data_batch_size: Optional[int] = field(default=9,metadata={ "help": ("For debugging purposes or quicker training, truncate the number of training examples to this ""value if set.")},)
...
def main():
    ...
    train_view_dataset = view_dataset(csv_path=data_args.view_data_csv_path)
    train_view_dataloader = DataLoader(train_view_dataset,batch_size = data_args.view_data_batch_size, shuffle=True) 
    ...
    if model_args.prompt_tuning:
        model = PromptCLIPModel.from_pretrained(model_args.model_name_or_path,
                                                cache_dir=model_args.cache_dir,
                                                revision=model_args.model_revision,
                                                token=model_args.token,
                                                trust_remote_code=model_args.trust_remote_code,)
        #model.setup_prompt_embedding(img_token_length=16,text_token_length=16)
        print("use my prompt CLIP model")
        ...
        '''
        Non-FINETUNING
        CIFAR100 Accuracy with 1000 samples is : 0.93
        Crawling Data Accuracy:0.48333333333333334
        Control View Data Accuracy:0.5472972972972973
        '''
    ...
    if model_args.prompt_tuning:
        ...
        model.setup_prompt_embeddings(vision_len=8,text_len=8)
        print("............Setup Prompt Tuning Model............")
        #model.eval()

    else:
        if model_args.freeze_vision_model or model_args.prompt_tuning:
            _freeze_params(model.vision_model)
            print("............Freezing Vision Model............")

        if model_args.freeze_text_model or model_args.prompt_tuning:
    ...
    print("image_size:{} mean:{} std:{}".format(config.vision_config.image_size, image_processor.image_mean, image_processor.image_std))
